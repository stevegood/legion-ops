...
export default function EditMatchModal({
  ...
  return (
    <Modal open={open} onEscapeKeyDown={onCancel}>
      <Paper>
        <Typography variant="h2">Edit Match</Typography>
            ...
            <Select value={player1} onChange={handlePlayerChange(setPlayer1)}>
              {event.players.map(player => (
                <MenuItem
                  ...
                  value={player.id}
                  disabled={!playerIsValidForSelection(player, player2, round)}
                >
                  {player.name}
                </MenuItem>
              ))}
            ...
            <Select value={player2} onChange={handlePlayerChange(setPlayer2)}>
              {event.players.map(player => (
                <MenuItem
                  ...
                  value={player.id}
                  disabled={!playerIsValidForSelection(player, player1, round)}
                >
                  {player.name}
                </MenuItem>
              ))}
